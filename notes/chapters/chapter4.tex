...
\par It looks rather smooth and continuous to me, not at all discrete.
...
\par It is customary to bestow a name on $\sigma^{ij}$: the \emph{stress tensor}. Curiously enough, the conservation of total angular momentum implies that the the stress tensor must be \emph{symmetric}. These rather nice properties might be ringing a bell in your memory. 
...
\par However, we'll work under the assumption that the bubble is always in pressure equilibrium with the surrounding fluid and does not exchange energy with it\,\footnote{Typically, pressure imbalances are removed rather quickly by \emph{acoustic waves}, while heat exchange takes more time. In short, we'll assume $P^{*}=P'$.}.
