...
\par We know that in the real world an abstract concept like a continuum cannot possibly exists: Matter is discrete and discrete only. However, it may have occurred to you that normally we don't exactly see matter broken down to its fundamental components.
\par We're able to regard a fluid as a continuum when the measured fluid property is constant for sensitive volumes small on the macroscopic scale but large in respect to the microscopic ones.
\par When looking at a fluid, we can distinguish two kinds of forces\footnote{Note that the forces proposed are to be intended \emph{per unit mass}.} that act on matter in bulk
\begin{itemize}
...
\begin{equation*}
    \sigma^{i} = \sigma^{ij}n_{j}
\end{equation*}
where the index placement is definitely overkill, but we'll be clearer in a moment. Here and in the following, we'll be often adopting Einstein convention for summation over repeated indices.
\par It is customary to bestow a name on $\sigma^{ij}$: The \emph{stress tensor}. Curiously enough, the conservation of total angular momentum implies that the the stress tensor must be \emph{symmetric}. This might be ringing a bell in your memory. 
\par If we put ourselves in the fluid's rest frame, where there's no net flux of any of the components of momentum in any of the three orthogonal directions, it's not a far stretch to be claiming that the fluid must, indeed, be isotropic at rest.
\par Isotropy implies that, at least in this reference frame, the stress tensor must be diagonal. Since the symmetry of the system is essentially spherical, all the spatial components $\sigma^{ii}$ must therefore be equal. The most general form for the 4-dimensional stress tensor is simply the energy-momentum tensor for a perfect fluid (as far as special relativity is concerned)
\begin{equation}
    T^{\mu\nu} = (\rho + p)u^{\mu}u^{\nu}+p\eta^{\mu\nu}
    \label{eq:emtensorpf}
\end{equation}
\begin{equation*}
    ...
    \end{pmatrix}
\end{equation*}
Note that we'll be using the same metric convention of \cite{carroll}, which is $\eta_{\mu\nu} = \text{diag}(-1,1,1,1)$ and we will work in units $c=1$\footnote{To properly reintroduce $c$ factors, you have to look carefully at the definition of $u^{\mu}$. Here, and only here, with $\rho$ we're referring to an \emph{energy density} and not to a \emph{matter density}.}. The 4-vector $u^{\mu}$ is the 4-velocity $u^{\mu} = \text{d}x^{\mu}/\text{d}s$. Please note that requiring the energy-momentum tensor to be locally conserved $\partial_{\mu}T^{\mu\nu} = 0$ allows us in principle to deduce the three fundamental equations of fluid dynamics: The mass conservation, the energy conservation and Euler's equation. We will, however, get to those equation taking another route, passing briefly from statistical mechanics. For those interested, you can see \cite{carroll}, §1.9.
\par The stress tensor we've been discussing so far is the solely spatial component of $T^{\mu\nu}$, which is $\sigma^{ij}=-T^{ij} = -p\delta^{i}_{j}$. Since the stress tensor is isotropic, all its diagonal elements are equal and then we may just take 
\begin{equation*}
...
\begin{equation}
    ...
    \label{eq:fluideq}
\end{equation}
If $\vec{F}$ is a conservative force $\vec{F} = -\nabla\phi$, then the condition for mechanical equilibrium may be cast in an equivalent form by taking the curl of both members
\begin{equation*}
...
\par For example, for a self-gravitating medium, the following equation holds (in the Newtonian limit at least)
\begin{equation}
    \nabla^{2}\phi = 4\pi G\rho 
    \label{eq:poissongrav}
\end{equation}
which can be suitably rearranged, recalling that $\nabla\phi = -\nabla p/ \rho$, into (we assume spherical symmetry)
\begin{equation}
...
\begin{equation*}
    M = \int_{0}^{R} 4\pi r^{2}\rho\,\text{d}r = 4\pi a^{3}\rho_{c}\int_{0}^{\eta_{1}}\eta^{2}\phi^{n}\,\text{d}\eta
\end{equation*} 
...
\begin{equation}
    \vec{\Omega}\wedge(\vec{\Omega}\wedge\vec{r}) = -\frac{1}{2}\nabla(\vec{\Omega}^{2}\,\vec{r}\,^{2}) = -\nabla \phi_{C}
    \label{eq:scalarcentrifugal}
\end{equation}
This means that the effective potential is no longer that granting spherical symmetry (say, the usual Newtonian gravitational potential). The equation for mechanical equilibrium will have to be modified into 
\begin{equation*}
    -\rho\nabla(\phi-\phi_{C}) = \nabla p
\end{equation*}
...
\par Now, if we are in presence of a temperature gradient, a sensible call would be remembering of the existence of \emph{thermal conduction}, which is temperature-gradients driven. This implies the presence of blobs of hot gas (or whatever) moving towards the cooler regions (Fig.\ref{fig:ellipse}). At this point, we'll no longer be able to neglect the Coriolis force, which will further twist the surface of the star.
\par It's clear that the easy, spherical hydrostatic equilibrium is not sustainable in these conditions.
\section{Vlasov equation}
What we'd like to do in the present section is trying to build a formalism that allows to connect the microscopical interpretation of matter through statistical mechanics to the macroscopic space.
...
\begin{equation}
    ...
    \label{eq:vlasov}
\end{equation}
This is called \emph{Collision-less Boltzmann equation} or \emph{Vlasov's equation}. Please, don't bother to ask why of the weird indices placement. Whichever way you put the indices is (here) absolutely irrelevant. 
...
\begin{align}
    ...
    \partial_{t}\left(\frac{1}{2}\rho v^{2}+\varepsilon \right) +\nabla\left(\rho\vec{v}\left[\frac{v^{2}}{2}+\varepsilon\right]\right) &= \vec{F}\cdot\vec{v}-\nabla(\vec{\phi}_{H}+p\vec{v}\,)
\end{align}
...
because now we have 
\begin{equation}
    \partial_{t}\left(\frac{1}{2}\rho v^{2}+\varepsilon\right) +\nabla\left(\rho\vec{v}\left[\frac{v^{2}}{2}+\varepsilon\right]\right) = \frac{\text{d}E}{\text{d}t}-\rho T\frac{\text{d}S}{\text{d}t} + \Lambda(T,\rho)
\end{equation}
...
So far, we've been blatantly ignoring the collision integral in Vlasov's equation saying that by the detailed equilibrium principle, at equilibrium its contribution is approximately zero. Clearly, this can't be always the case, even more so in fluids.
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        ...
        \caption{Sketch of dilatation or compression.}
    \end{figure}
...
\par Generally, the deviatoric stress is expressed by means of a fourth rank tensor\footnote{It may be worth asking yourself if $A$ is truly a 4-index tensor or just a 4-index symbol.}
\begin{equation*}
...
\begin{equation}
    \sigma_{ij} = -p\delta_{ij}+2\mu\left(\frac{1}{2}\left[\partial_{j}u_{i}+\partial_{i}u_{j}\right]-\frac{1}{3}\nabla\cdot\vec{u}\delta_{ij}\right)
...
\par From eq.\ref{eq:stresstensor} we reckon that if the fluid is incompressible ($\nabla\cdot\vec{u} = 0$), the Navier-Stokes equation has the form 
\begin{equation}
...
\begin{equation*}
    \partial_{t}\left(\frac{1}{2}\rho v^{2}+\varepsilon\right) +\nabla\left(\rho\vec{v}\left[\frac{v^{2}}{2}+\varepsilon\right]\right) - \vec{F}\cdot\vec{v}= 0 
\end{equation*}
...
\begin{equation}
    ...
    \label{eq:bernoulli}
\end{equation}
which is just a restating of conservation of energy. In the following section we'll try examining one simple application of (\ref{eq:bernoulli}).
...
\par If the fluid is incompressible, Bernoulli's theorem requires that 
\begin{equation*}
    \frac{\rho v^{2}}{2} + p = \text{const.}
\end{equation*}
which has as a natural consequence that given two sections of areas $A$ and $a$ with velocity $\vec{v}$ at $A$ (see Fig.\ref{fig:nozzle}), upon reaching the point where the section becomes $a$, the velocity becomes 
\begin{equation*}
...
\end{equation*}
To properly solve the equations, we need to introduce an Equation of State\footnote{Sometimes I may be using EoS as a more compact notation.} of some sort. Here (and rather often in the following) we'll assume a barotropic relation between pressure and density 
\begin{equation}
    p(\rho) = c_{s}^{2}\rho
    \label{eq:barotropic}
\end{equation}
...
\begin{align*}
    \rho v \partial_{x}v &= -c_{s}^{2}\partial_{x}\rho\\
    v\partial_{x}v &= c_{s}^{2}\left(\frac{\text{d}v}{\text{d}x}\frac{1}{v}+\frac{\text{d}A}{\text{d}x}\frac{1}{A}\right)\\
...
Suppose we have a stratified plane parallel atmosphere approximately in hydrostatic equilibrium. Consider now a blob of fluid which has been displaced upwards as shown in the picture below.
\par Initially, the blob has the same density and pressure as the surrounding fluid, respectively $\rho_{0},\,P$. After the displacement, the external fluid density and pressure are $\rho',\,P'$ which are, in principle, different from density and pressure inside the bubble, $\rho^{*},\,P^{*}$.
\par However, we'll work under the assumption that the bubble is always in pressure equilibrium with the surrounding fluid and does not exchange energy with it\,\footnote{Typically, pressure imbalances are quickly removed by \emph{acoustic waves}, while heat exchange takes more time. In short, we'll assume $P^{*}=P'$.}.
\input{chapters/bubble}
\par Let's consider the infinitesimal displacement $\varepsilon$, so that the equation of motion is
\begin{equation*}
...
\end{align*}
Plugging these two equations into the equation of motion yields a familiar expression 
\begin{equation}
...
\par How good is the "bubble picture" we've just used? Not much actually, as we'll see in the next chapter, but it still allows to get a grasp of how convection may work.
...
\par To make things a little simpler, we're going to assume that the star rotates with angular velocity $\vec{\Omega}$ \textbf{as a rigid body}. This is a rather poor assumption, that may be true only for the core region of stars; in general, \emph{differential rotation} applies, so $\Omega = \Omega(r,\theta,t)$.
...
\begin{equation}
    \frac{\text{D}\vec{v}}{\text{D}t} +2\vec{\Omega} \wedge \vec{v} = -\frac{\nabla p}{\rho}+\vec{F}
\end{equation}
Here $2\vec{\Omega} \wedge \vec{v}$ represents Coriolis' forces\,\footnote{Coriolis' forces arise only in presence of latitudinal motion and imply the presence of shear.}. At this point it's convenient to define a new quantity, the \emph{vorticity} as $ \vec{\omega} = \nabla \wedge \vec{v}$.
...
\subsubsection*{Useful vectorial Identity}
It may be useful to recall the following vectorial identity:
\begin{equation*}
    \nabla(\vec{A}\cdot\vec{B}) = (\vec{A}\cdot\nabla)\vec{B}+(\vec{B}\cdot\nabla)\vec{A}+\vec{A}\wedge (\nabla\wedge\vec{B})+\vec{B}\wedge (\nabla\wedge\vec{A})
\end{equation*}
which for $\vec{A} = \vec{B}$ reduces to
\begin{equation*}
    (\vec{A}\cdot\nabla)\vec{A} = \frac{1}{2}\nabla(\vec{A}\cdot\vec{A}) - \vec{A}\wedge (\nabla\wedge\vec{A})
\end{equation*}
If $\vec{A}$ can be written as the gradient of a scalar potential, the last term is notably zero. This identity can be most useful to restate Euler's (Navier-Stokes') equation in an equivalent form, which we'll make use in the next chapter.