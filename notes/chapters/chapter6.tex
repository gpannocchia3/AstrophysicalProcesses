...
\end{equation*}
In this form it's easy to see that if $v>c_{s}$ the fluid must be accelerating\footnote{It dawned on me that stellar winds are much similar to a spherically symmetric de Laval's nozzle (Fig.\ref{fig:nozzle}) with increasing section (the spherical shells). I'm pretty sure this was mentioned during class, though.}.
...
\begin{equation}
    ...
    \label{eq:fake_schw}
\end{equation}
which is commonly known as Bondi radius. Note that the order of magnitude of $r_{s}$ is about 100 times that of the solar corona, which is much larger than the radius $R_{*}$ of any compact object. Technically, a very high temperature would be needed in order to make $r_{s}<R_{*}$. This condition can be achieved, for example, if we consider a standing shock wave close to the stellar surface.
...
\par It should be pretty obvious that, as the gas infalls into the star, it builds up energy. This process is eventually brought to a stop if the star has a \emph{hard boundary} at, say, $r = \bar{r}$.
\par For a body of mass $M$ and radius $\bar{r}$, the gravitational potential energy released by the accretion of a mass $m$ onto its surface is (assuming gas starting from infinity) $\Delta E_{\text{acc}} = GMm/\bar{r}$. We would expect this energy to be released eventually mainly in the form of electromagnetic radiation.
\par It is clear from the form of the last expression that the efficiency of accretion as an energy release mechanism is strongly dependent on the compactness of the accreting object.
\par The energy dissipated, either by radiation or by the gas heating up, if the gas doesn't stop, follows from application of the Virial theorem $\Delta E_{\text{acc}} = GMm/2\bar{r}$.
\par Let us assume for the moment adiabatic accretion. The internal energy of the gas could then be expressed as 
\begin{equation*}
    e = \frac{p}{(\gamma-1)\rho} \quad p = \frac{\rho k T}{\mu m_{H}}
\end{equation*}
...
\begin{equation}
    L_{E} = 4\pi r^{2}F_{\nu, \,eq} = \frac{4\pi c GMm_{p}}{\kappa_{\nu}}
