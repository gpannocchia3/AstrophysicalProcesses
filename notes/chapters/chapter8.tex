...
\par In short, unless pointed out otherwise, our diskussion will be classical.
\subsubsection{A short detour on redshift}
In our description of the Universe we'd like to observationally determine a number of quantities to try to infer which of the models proposed by the FRW metric\footnote{As per any GR subject in this notes, I'll follow \cite{carroll}. You can find this diskussion at §8.5.}
\begin{equation}
...
cluster) and the second describes the gravitational energy that comes through the interaction of the two popolations; $r_{i}$ is the half-mass radius of population.
...
\par Another possibility for the binary to increase the binding energy during a 3-body interaction is the exchange, for example if the intruder replaces one of the members of the binary. This usually happens when $M_{2}<m_{3}<M_{1}$, in which case, after the exchange the binary is formed by $M_{1}$ and $m_{3}$
\begin{equation*}
...
We now turn to diskuss some relevant astrophysical aspects of (S)MBHs. Those objects has been observed at the centre of massive galaxies, and inhabit virtually all nuclei of galaxies with $M_{*} > 10^{11}M_{\odot}$, whereas their ubiquity in lighter galaxies is much debated. Our diskussion will be mainly classical (Newtonian), although some results of GR will be implemented when necessary.
\subsection{Bondi accretion}
In order to accrete, a MBH needs to capture gas from its surroundings at a suﬃcient rate. We've already diskussed about this in Chapter \ref{ch:vi}, but a brief refresher won't do much harm.
...
We've shown how, given the existence of a seed, we're able to accrete a BH up to a MBH, but we've never diskussed about what actually are these seeds we're talking about.
...
\end{equation*}
Once the two BHs overlap with the cloud's center of mass, DF cannot bring them any closer. To do so, we need Stellar Hardening and all the other nice processes we've diskussed earlier.
\begin{flushright}
    \emph{The End?}
\end{flushright}